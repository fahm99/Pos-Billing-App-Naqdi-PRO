...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('الدفع', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
          centerTitle: true,
                  ...
                  children: [
                    Expanded(
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          children: [
                            _buildItemsTable(billingState, borderColor, currencySymbol),
                            const SizedBox(height: 16),
                            _buildDiscountTaxSection(subtotal, currencySymbol),
                            const SizedBox(height: 16),
                            _buildCustomerSelector(),
                            const SizedBox(height: 16),
                            _buildPaymentSection(total, currencySymbol),
                            const SizedBox(height: 16),
                            if (upiId.isNotEmpty &&
  ...
  Widget _buildItemsTable(BillingState state, Color borderColor, String currencySymbol) {
    return Container(
        ...
        child: Table(
          ...
          children: [
            ...
            ...state.cartItems.map((item) => TableRow(children: [
                  _dataCell('${item.quantity} × ${item.product.name}', TextAlign.right),
                  _dataCell('$currencySymbol${item.product.price.toStringAsFixed(2)}',
                      TextAlign.right, isSubtitle: true),
                  _dataCell('$currencySymbol${item.total.toStringAsFixed(2)}', TextAlign.right,
                      isBold: true),
                ])),
          ],
        ),
  ...
  Widget _buildDiscountTaxSection(double subtotal, String currencySymbol) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(12),
        ...
        children: [
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('خصم ($currencySymbol)', style: TextStyle(fontSize: 12, color: Colors.grey[600])),
          ...
          if (_discountAmount > 0 || _taxPercent > 0) ...[
            ...
            Row(
              ...
              children: [
                Text('المجموع الفرعي', style: TextStyle(fontSize: 12, color: Colors.grey[500])),
                Text('$currencySymbol${subtotal.toStringAsFixed(2)}', style: const TextStyle(fontSize: 12)),
              ],
            ),
            if (_discountAmount > 0) ...[
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('الخصم', style: TextStyle(fontSize: 12, color: Colors.red[400])),
                  Text('-$currencySymbol${_discountAmount.toStringAsFixed(2)}',
            ...
            if (_taxPercent > 0) ...[
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('ضريبة $_taxPercent%', style: TextStyle(fontSize: 12, color: Colors.grey[500])),
                  Text('+ $currencySymbol${((subtotal - _discountAmount) * _taxPercent / 100).toStringAsFixed(2)}',
              ...
              Expanded(
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<Customer?>(
                    ...
                    hint: const Text('اختر عميل (اختياري)', style: TextStyle(fontSize: 13)),
                    isExpanded: true,
                    items: [
                      const DropdownMenuItem<Customer?>(value: null, child: Text('بدون عميل')),
  ...
  Widget _buildPaymentSection(double total, String currencySymbol) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(12),
            ...
            if (_paymentMethod == PaymentMethod.cash && _cashPaid > 0) ...[
              ...
              Container(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text('الباقي للعميل', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                    Text(
                      '$currencySymbol${(_cashPaid - total).toStringAsFixed(2)}',
                      style: TextStyle(
                          fontWeight: FontWeight.bold,
                          fontSize: 15,
                          color: _cashPaid >= total ? Colors.green : Colors.red),
                    ),
                  ],
                ),
              ),
  ...
  Widget _buildQrSection(String upiId, String shopName, double total) {
    return Container(
      ...
      child: Column(
        children: [
          const Text('امسح للدفع', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
          const SizedBox(height: 12),
          SizedBox(
            width: 160,
            height: 160,
            child: PrettyQrView.data(
              data: 'upi://pay?pa=$upiId&pn=$shopName&am=${total.toStringAsFixed(2)}&cu=SAR',
            ),
          ),
        ],
      ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_paymentMethod == PaymentMethod.cash && _cashPaid > total)
            Padding(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('الباقي', style: TextStyle(fontSize: 12, color: Colors.grey[500])),
          ...
          PrimaryButton(
            onPressed: () => context.push('/print-options'),
            label: 'متابعة للدفع',
